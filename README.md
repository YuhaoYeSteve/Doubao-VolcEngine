# 豆包 App-火山方舟版

![Last Commit](https://img.shields.io/github/last-commit/YuhaoYeSteve/Doubao-VolcEngine?style=flat-square&color=5D5FEF) ![Commit Activity](https://img.shields.io/github/commit-activity/m/YuhaoYeSteve/Doubao-VolcEngine?style=flat-square&color=3B82F6) ![Repo Size](https://img.shields.io/github/repo-size/YuhaoYeSteve/Doubao-VolcEngine?style=flat-square&color=10B981)

> 本项目完全基于 Trae 进行 Vibe Coding 构建。

本项目基于火山引擎的 **火山方舟 (Volcengine Ark)** 平台，通过集成 **Python SDK** 来调用 **Doubao (豆包)** 大模型。

为了直观展示模型调用能力，项目中包含了一个简易的前端演示界面。该界面实现了与豆包大模型的多轮对话功能，帮助开发者快速理解如何通过 SDK 接入并使用火山方舟的 AI 能力。

> **📚 相关文档**
>
> - [火山方舟 (Volcengine Ark) 产品主页](https://www.volcengine.com/product/ark)
> - [火山方舟 API 文档](https://www.volcengine.com/docs/82379/1099475)
> - [豆包大模型介绍](https://www.volcengine.com/product/doubao)

![App Preview](assets/screenshot-20260209-193208.png)

## ✨ 功能特性

- **多轮对话**：支持与 Ark 大模型（如 Doubao-pro/lite）进行上下文连贯的对话。
- **联网搜索**：集成火山引擎 Web Search 插件，支持实时联网获取信息，并展示搜索过程与思考状态。
- **会话管理**：
  - 左侧侧栏管理历史会话。
  - 支持新建、切换、重命名和删除会话。
  - 会话数据保存在本地浏览器（LocalStorage），隐私安全。
- **界面友好**：
  - 仿豆包风格的 UI 设计。
  - 响应式布局，适配桌面与移动端。
  - 实时显示 Token 消耗情况。
  - **灵活配置**：支持在前端界面直接配置 API Key 和 Model ID，无需重启服务即可切换环境。
- **易于扩展**：后端使用 FastAPI，结构清晰，方便二次开发。

## 🛠️ 技术栈

- **后端**：Python, FastAPI, Uvicorn, Volcengine Python SDK
- **前端**：HTML5, Tailwind CSS (CDN), Vanilla JavaScript

## 🚀 快速开始

### 1. 克隆项目

```bash
git clone https://github.com/YuhaoYeSteve/Doubao-VolcEngine.git
cd Doubao-VolcEngine
```

### 2. 安装依赖

确保你安装了 Python 3.9+。

```bash
pip install -r requirements.txt
```

### 3. 配置 API Key

你可以选择以下任意一种方式配置：

**方式 A：前端配置（推荐）**
启动项目后，点击左侧边栏底部的“更多”按钮，在设置弹窗中填入 API Key 和 Model ID 即可即时生效。

**方式 B：后端配置文件**
1. 复制配置文件模板：
   ```bash
   cp config.example.ini config.ini
   ```
2. 编辑 `config.ini`，填入你的 API Key：
   ```ini
   [ARK]
   api_key = 你的_API_KEY
   ```
   *或者，你也可以通过环境变量设置：*
   ```bash
   export ARK_API_KEY=你的_API_KEY
   ```

### 4. 启动服务

```bash
python -m uvicorn ark_server:app --host 0.0.0.0 --port 8000
```

### 5. 访问应用

打开浏览器访问：[http://localhost:8000](http://localhost:8000)

## 📂 项目结构

```
.
├── ark_server.py      # 主后端服务 (FastAPI)
├── chat.html          # 主前端页面
├── config.ini         # 配置文件 (需自行创建)
├── config.example.ini # 配置文件模板
├── requirements.txt   # 项目依赖
└── assets/            # 静态资源 (图片等)
```

## 📝 备注

- 本项目仅供学习和演示使用。
- 前端使用 CDN 加载 Tailwind CSS，请确保网络通畅。
- 默认使用的模型接入点为 `doubao-seed-1-8-251228`，如需更改请修改 `ark_server.py` 中的 `model` 参数。

## 🚧 最近开发动态 (Auto)

<!--START_SECTION:recent_commits-->

### 最新 5 次提交记录

- [`9544576`](https://github.com/YuhaoYeSteve/Doubao-VolcEngine/commit/9544576e3043d018ad9e867fc5b32e430a0694e6) docs: release v5.4.0 - frontend refactor and design system (2026-02-22)
- [`2c66eef`](https://github.com/YuhaoYeSteve/Doubao-VolcEngine/commit/2c66eef72265b3c8ef8a52692b297d3574ecb02d) feat: release v5.4.0 with frontend refactoring and mobile support (2026-02-19)
- [`40138ad`](https://github.com/YuhaoYeSteve/Doubao-VolcEngine/commit/40138ad77e4281f084a6bed42ec3bebda1c5c9c1) docs: add badges and auto-update workflow for readme (2026-02-12)
- [`dfe059d`](https://github.com/YuhaoYeSteve/Doubao-VolcEngine/commit/dfe059da59af4ff943db8f51496e1f9564460251) feat: optimize chat streaming and scroll ux (v5.3.0) (2026-02-11)
- [`a6ed3af`](https://github.com/YuhaoYeSteve/Doubao-VolcEngine/commit/a6ed3af297aae6072b2eba95d7fcefa902266a01) feat(v5.2.0): add web search citations and improve scroll UX (2026-02-11)
> 自动更新于 2026-02-22 03:25:21

<!--END_SECTION:recent_commits-->

## 📅 更新日志

- **v5.4.0** (2026-02-19)
  - ⚡️ **架构重构**：将前端代码拆分为独立的 CSS/JS 文件，提升项目可维护性与加载性能。
  - 🎨 **设计规范**：引入 CSS 变量 (Design Tokens) 统一管理颜色与视觉风格，告别硬编码。
  - 📱 **移动端适配**：新增侧边栏响应式支持，优化移动设备上的交互体验。
  - 🔧 **后端升级**：后端服务支持静态资源文件挂载，为前端分离提供基础。

- **v5.3.0** (2026-02-11)
  - 🎞️ 视觉体验：实现真·打字机效果（60FPS 渲染循环），彻底解决流式输出时的卡顿和蹦字现象。
  - 🖱️ 交互优化：重构滚动判定逻辑（20px 阈值），实现“即停即止”的丝滑滚动控制，不再抢夺用户视线。
  - 🐛 修复：解决了因渲染节流导致的流式输出在视觉上丢失或延迟的问题。
  - 🔧 稳定性：后端增加详细的错误堆栈打印，便于排查 500/401 等认证错误。

- **v5.2.0** (2026-02-10)
  - 🔍 搜索增强：自动为联网搜索结果注入引用提示词，AI 回答将包含可点击的来源链接。
  - 📜 交互优化：优化前端消息流式渲染时的滚动体验，支持生成过程中自由滑动浏览历史消息，智能跟随底部。

- **v5.1.0** (2026-02-10)
  - ⚙️ 配置增强：在“更多”设置中新增 API Key 和 Model ID 配置项，支持前端动态切换。
  - 🔧 优先策略：请求优先使用前端配置的 Key/Model，未配置时回退至后端环境变量或配置文件。

- **v5.0.0** (2026-02-10)
  - 🛠️ 架构重构：统一后端 API 接口，所有请求（对话/搜索）均通过 `responses` 接口处理，提升系统稳定性与扩展性。
  - ⚡️ 性能优化：移除了冗余的流式处理分支，统一了事件流解析逻辑。
  - 🐛 修复：解决了因接口分流导致的潜在状态不一致问题。

- **v4.0.1** (2026-02-10)
  - 📝 文档：更新项目仓库地址为 `Doubao-VolcEngine`。

- **v4.0.0** (2026-02-10)
  - 🚀 重磅更新：集成火山引擎联网搜索 (Web Search) 能力，支持实时获取互联网信息。
  - 📛 品牌升级：项目正式更名为“豆包 App-火山方舟版”。
  - ✨ 体验优化：
    - 新增“思考中”与“正在搜索”的状态可视化，让 AI 的行动更透明。
    - 优化长文本与复杂流式响应的解析逻辑，彻底解决卡顿问题。
  - 🔧 架构升级：支持自定义 Endpoint ID，灵活适配各类插件化模型配置。

- **v3.0.2** (2026-02-06)
  - 🗑️ 简化：移除设置中的 Model Endpoint ID 输入框，统一使用后端配置，减少用户配置负担。

- **v3.0.1** (2026-02-06)
  - 🐛 修复：后端服务重启以正确支持图片上传的数据结构。
  - ⚡️ 优化：前端增加错误消息的气泡提示，避免请求失败时无响应。

- **v3.0.0** (2026-02-05)
  - 🎨 极致还原：深度复刻豆包 (Doubao) 网页版 UI，从配色到布局完全重构。
  - 🖥️ 侧边栏：移除突兀的蓝色按钮，采用极简灰白风格，增加“AI 搜索/创作”入口占位。
  - 💬 对话体验：
    - 用户气泡改为清新的淡蓝色 (`#EBF5FF`)，文字深色高对比度。
    - AI 回复去除气泡背景，采用文档式沉浸排版，更适合阅读长文。
  - ⌨️ 输入增强：悬浮式输入框，底部集成功能工具栏（深度思考、联网搜索等），视觉效果对标原生应用。

- **v2.2.2** (2026-02-05)
  - 💎 UI 细节优化：
    - 调整主色调为更通透的琉璃蓝 (Blue-500/80)。
    - 为按钮和气泡增加细腻的玻璃边框和模糊效果。
    - 优化“新建对话”按钮和发送按钮的视觉一致性。

- **v2.2.2** (2026-02-05)
  - ✨ 视觉增强：输入框的光影光晕效果改为常驻显示，增强界面层次感和现代感。

- **v2.2.1** (2026-02-05)
  - 🎨 样式调整：回归纯白背景，保持界面清爽。
  - ✨ 交互优化：输入框增加聚焦光晕特效 (Glowing Effect)，提升输入沉浸感。
  - 🔄 回滚：取消全局 Glassmorphism 背景，仅保留输入框的毛玻璃质感。

- **v2.2.0** (2026-02-05)
  - 💎 UI 焕新：采用 Glassmorphism（毛玻璃）设计语言，整体风格更加轻盈通透。
  - 📛 品牌升级：项目名称变更为“你的豆包”。
  - 🎨 细节优化：调整蓝色色调为更柔和的透光蓝，增加背景纹理和阴影层次。

- **v2.1.0** (2026-02-05)
  - 🎨 UI 重构：全新仿豆包 (Doubao) 界面设计。
  - ✨ 视觉升级：
    - 侧边栏改为清爽的浅灰色风格，优化列表交互。
    - 底部输入框改为悬浮卡片式设计，提升现代感。
    - 消息气泡采用经典的蓝(用户)/灰(AI)配色，圆角更自然。
    - 整体布局优化，消息区域最大宽度限制，阅读体验更佳。

- **v2.0.1** (2026-02-05)
  - 🐛 修复：解决 Markdown 样式覆盖导致的气泡背景色丢失问题。

- **v2.0.0** (2026-02-05)
  - 🌟 重大更新：Markdown 渲染与代码高亮支持。
  - 🖼️ 多模态：支持上传/粘贴图片，与 Vision 模型交互。
  - ⚙️ 设置：新增配置面板，支持自定义 System Prompt 和 Model Endpoint。
  - 📋 体验：代码块一键复制，输入框自动增高。

- **v1.1.1** (2026-02-04)
  - 🎨 优化：新增 AI 回复前的“正在输入”动画（三个跳动的点）。

- **v1.1.0** (2026-02-04)
  - ✨ 新增：流式输出（打字机效果），提升对话体验。
  - 🔒 优化：在 AI 回复生成过程中禁用输入框和发送按钮，防止重复提交。

- **v1.0.0** (2026-02-04)
  - 🎉 初始发布：基于 Volcengine Ark SDK 的豆包大模型对话演示。
  - 💬 功能：支持基本的多轮对话、会话管理。
